...
### Breaking Changes
- #15 misc updates: POST /cart/items now adds updatedAt to its response. Update response schemas, generated clients, and exact contract tests to accept the field before upgrading.
- #6 feat: bulk export endpoint: GET /v1/export was removed and replaced by POST /v2/exports/bulk. The new HTTP 202 response contains exportId, status, format, and links.status. Run the required export migration and update clients before upgrading. The PR does not provide the migration command, so confirm the procedure with the change owner before rollout.
